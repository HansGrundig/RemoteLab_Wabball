...
> Kurze Einführung in Edrys Lite:
> Nach dem öffnen des Links, kommst du direkt in die Lobby. 
> Wenn du oben links auf das hamburger-Menü-Icon klickst, kannst direkt in die Stationansicht springen und mit den Aufgaben starten.
...
- IMU: MPU6050 6-Achsen Gyroskop und Beschleunigungssensor:
  - weitere Informationen: [MPU6050](https://components101.com/sensors/mpu6050-module)
...
> [!Note] 💡 Hinweis:
> Über die Funktion `sendNextionCommand()` kannst du Nextion-Befehle an das Display senden. Der Befehl ist in der `NextionControl.h`-Bibliothek implementiert.
>
>```cpp
...
void loop() {
    char cmd[64];
    .....// Befehl zum Zeichnen eines blauen Punktes in der Mitte des Displays erstellen als String
...
Warum müssen die Servos in der `setup()`-Methode mit der Funktion `initJiggle(pinS1,pinS2,pinS3)` initialisiert werden:

- [[X]] Damit die Servos korrekt mit den richtigen Pins verbunden und für `jiggle()` konfiguriert sind.
- [[ ]] Damit die Servos die richtige Geschwindigkeit haben.
- [[X]] Damit die Servos sich auf Null-Position zurücksetzen.
- [[ ]] Damit die Servos die richtige Richtung haben.
...
>* Immer dort einen **schwarzen** Punkt zeichnen, wo die Kugel das Display berührt.
...
>[!TIP] 🎯 Die Bewegung der Kugel als Spur darstellen.
...
Nach welchen Prinzip wird der Puffer der letzten 10 Punkte verwaltet?:
...
Damit der smarte Billardtisch die Bewegung der Kugel analysieren kann, soll er die Geschwindigkeit und Bewegungsrichtung der Kugel berechnen.
Gib dafür über die serielle Schnittstelle (`Serial`) kontinuierlich aus:
...
> [!Tip] 🎯 Bewegungsdaten der Kugel auswerten.
...
Warum wird die Geschwindigkeit und Richtung der Kugel über die letzten 5 Punkte berechnet?

- [[X]] Weniger Rechenaufwand, da nicht jeder Punkt einzeln betrachtet werden muss.
- [[ ]] Das System kann nur 5 Punkte gleichzeitig speichern.
- [[X]] Durch Mittelung über mehrere Punkte werden Ausreißer und Messfehler reduziert, was zu einer stabileren Berechnung führt.
- [[ ]] Die Geschwindigkeit und Richtung ändern sich nicht innerhalb von 5 Punkten.

## Bonusaufgabe 5: Position vorhersagen
Damit der smarte Billardtisch die Bewegung der Kugel vorhersagen kann, soll er die **stillstands Position** der Kugel berechnen.
Schreibe dazu eine Funktion, die unter Verwendung der aktuellen Position, Geschwindigkeit und Richtung die **diese Position**  berechnet. 
...
>[!TIP] 🎯  Vorhersage von der Kugelpositionen
...
Warum ist die Vorhersage der nächsten Kugelposition kein exakter Wert, sondern nur eine Näherung?

- [[ ]] Die Vorhersage ist exakt, da die Physik-Engine alle Einflüsse berücksichtigt.
- [[X]] Weil das System mit einem Modell arbeitet, das reale Einflüsse nur vereinfacht abbildet
- [[X]] Die Vorhersage basiert auf den letzten Berührungspunkten, die nicht immer die exakte Bewegung der Kugel widerspiegeln.
- [[ ]] Weil die Servos die Position zufällig verändern.
